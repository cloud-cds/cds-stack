...
SELECT 
	...
	,mar.INFUSION_RATE,themedunit.NAME InfusionRateUnit, mar.mar_duration DurationToInfuse, 
	CASE WHEN mar.mar_duration_unit_c = 1 then 'minutes' when mar.mar_duration_unit_c = 2 then 'HOURS' 
	WHEN MAR.MAR_DURATION_UNIT_C = 3 THEN 'days' END Duration_Infuse_Unit
	,mar.comments
	,MED.MIN_DISCRETE_DOSE
	,MED.MAX_DISCRETE_DOSE
FROM dbo.ORDER_MED MED LEFT JOIN dbo.CLARITY_MEDICATION medIndex on medIndex.MEDICATION_ID = med.MEDICATION_ID
