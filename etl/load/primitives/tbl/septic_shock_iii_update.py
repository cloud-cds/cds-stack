...
async def septic_shock_iii_update(fid, fid_input, conn, log, dataset_id = None, twf_table='cdm_twf'):
    ...
    from %(twf_table)s %(dataset_where_block)s ) c1
    join (select * from cdm_t where fid='suspicion_of_infection' %(dataset_block)s) c2
    ...
    """ % {'twf_table': twf_table, 'dataset_block': ' and dataset_id = %s' % dataset_id if dataset_id is not None else '', 'dataset_where_block': ' where dataset_id = %s' % dataset_id if dataset_id is not None else ''}
    ...
    """ % {'twf_table': twf_table, 'dataset_block': ' and %s.dataset_id = %s' % (twf_table, dataset_id) if dataset_id is not None else '', 'dataset_where_block': ' where dataset_id = %s' % dataset_id if dataset_id is not None else ''}
