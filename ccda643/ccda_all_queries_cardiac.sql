IF OBJECT_ID('analytics.dbo.CCDA643_CSNLookupTable_cardiac', 'U') IS NOT NULL
drop table analytics.dbo.CCDA643_CSNLookupTable_cardiac;
select pat_id "PAT_ID", pat_mrn_id "PAT_MRN_ID", csn "PAT_ENC_CSN_ID", csn "EXTERNAL_ID"
INTO
analytics.dbo.CCDA643_CSNLookupTable_cardiac
FROM
(SELECT DISTINCT pat_enc_hsp_1.PAT_ENC_CSN_ID, patient.PAT_ID, patient.PAT_MRN_ID
  ...
  -- ENSURE PATIENT WAS ADMITTED TO ONE OF THE HOWARD COUNTY UNITS YOU ARE STUDYING
    ...
    AND HOSP_ADMSN_TIME BETWEEN '2017-04-01' AND  '2017-05-01'
...
IF OBJECT_ID('analytics.dbo.CCDA643_PatLookupTable_cardiac', 'U') IS NOT NULL
drop table analytics.dbo.CCDA643_PatLookupTable_cardiac;
select pat_id "PAT_ID", pat_mrn_id "PAT_MRN_ID", IDENTITY_ID "EXTERNAL_ID"
INTO
analytics.dbo.CCDA643_PatLookupTable_cardiac
FROM
...
:OUT \\Client\F$\clarity-cardiac\adt.rpt
SET NOCOUNT ON
...
INNER JOIN Analytics.dbo.CCDA643_CSNLookupTable_cardiac csn ON CLARITY_ADT.PAT_ENC_CSN_ID = csn.PAT_ENC_CSN_ID
...
:OUT \\Client\F$\clarity-cardiac\demo.rpt
SET NOCOUNT ON
...
FROM CLARITY.dbo.PAT_ENC_HSP PAT_ENC_HSP_1
INNER JOIN CLARITY.dbo.PATIENT patient ON PAT_ENC_HSP_1.pat_id = patient.pat_id
INNER JOIN Analytics.dbo.CCDA643_CSNLookupTable_cardiac csn ON PAT_ENC_HSP_1.PAT_ENC_CSN_ID = csn.PAT_ENC_CSN_ID
INNER JOIN Analytics.dbo.CCDA643_PatLookupTable_cardiac pat ON pat.pat_id = pat_enc_hsp_1.pat_id
...
:OUT \\Client\F$\clarity-cardiac\diag.rpt
SET NOCOUNT ON
SELECT DISTINCT CSN.EXTERNAL_ID CSN_ID
  ,edg.DX_ID
  ,DX_ED_YN
...
FROM Analytics.dbo.CCDA643_CSNLookupTable_cardiac csn
...
:OUT \\Client\F$\clarity-cardiac\flt_lda.rpt
SET NOCOUNT ON
SELECT DISTINCT csn.EXTERNAL_ID CSN_ID
...
INNER JOIN Analytics.dbo.CCDA643_CSNLookupTable_cardiac csn ON pat_enc_hsp.PAT_ENC_CSN_ID = csn.PAT_ENC_CSN_ID
...
:OUT \\Client\F$\clarity-cardiac\flt_new.rpt
SET NOCOUNT ON
SELECT DISTINCT  csn.EXTERNAL_ID CSN_ID
  ,IP_FLO_GP_DATA.FLO_MEAS_NAME
  ,IP_FLO_GP_DATA.DISP_NAME
...
WHERE
  ...
  or IP_FLWSHT_MEAS.FLO_MEAS_NAME ilike '%nasal cannula%'
  or IP_FLWSHT_MEAS.FLO_MEAS_NAME ilike '%mask%'
  or IP_FLWSHT_MEAS.FLO_MEAS_NAME ilike '%O2 Delivery%'
  or IP_FLWSHT_MEAS.FLO_MEAS_NAME ilike '%O2 Device%'
  or IP_FLWSHT_MEAS.FLO_MEAS_NAME ilike '%settings%'
;
GO

:OUT \\Client\F$\clarity-cardiac\flt.rpt
SET NOCOUNT ON
SELECT DISTINCT  csn.EXTERNAL_ID CSN_ID
  ,IP_FLO_GP_DATA.FLO_MEAS_NAME
  ,IP_FLO_GP_DATA.DISP_NAME
...
:OUT \\Client\F$\clarity-cardiac\labs.rpt
SET NOCOUNT ON
SELECT csn.EXTERNAL_ID CSN_ID
  ,COMP.COMPONENT_ID
  ,COMP.BASE_NAME
...
:OUT \\Client\F$\clarity-cardiac\labs_new.rpt
SET NOCOUNT ON
SELECT csn.EXTERNAL_ID CSN_ID
  ,COMP.COMPONENT_ID
  ,COMP.BASE_NAME
...
:OUT \\Client\F$\clarity-cardiac\lda.rpt
SET NOCOUNT ON
SELECT pat.EXTERNAL_ID PAT_ID
  ...
  INNER JOIN
  Analytics.dbo.CCDA643_PatLookupTable_cardiac pat ON pat.PAT_ID = LDA.PAT_ID;
GO

USE CLARITY;
:OUT \\Client\F$\clarity-cardiac\mar.rpt
SET NOCOUNT ON
SELECT
...
inner join Analytics.dbo.CCDA643_CSNLookupTable_cardiac csn ON med.PAT_ENC_CSN_ID = csn.pat_enc_csn_id
...
:OUT \\Client\F$\clarity-cardiac\hist.rpt
SET NOCOUNT ON
SELECT DISTINCT csn.EXTERNAL_ID CSN_ID
...
LEFT OUTER JOIN analytics.dbo.CCDA643_CSNLookupTable_cardiac csn ON MedicalHistory.MINCSN = csn.PAT_ENC_CSN_ID
INNER JOIN analytics.dbo.CCDA643_PatLookupTable_cardiac pat ON pat.PAT_ID = MedicalHistory.PAT_ID
...
USE Analytics;
:OUT \\Client\F$\clarity-cardiac\ordermed.rpt
SET NOCOUNT ON
...
INNER JOIN
Analytics.dbo.CCDA643_CSNLookupTable_cardiac pat_enc_hsp_1 ON pat_enc_hsp_1.pat_enc_csn_id = med.PAT_ENC_CSN_ID
WHERE (
    med.IS_PENDING_ORD_YN = 'N'
    OR med.IS_PENDING_ORD_YN IS NULL
    )
...
USE Analytics;
:OUT \\Client\F$\clarity-cardiac\ordermed_home.rpt
SET NOCOUNT ON
SELECT DISTINCT PAT_ENC_HSP_1.EXTERNAL_ID CSN_ID
  ,med.display_name
  ,med.ORDER_INST
...
USE Analytics;
:OUT \\Client\F$\clarity-cardiac\orderproc.rpt
SET NOCOUNT ON
...
INNER JOIN CCDA643_CSNLookupTable_cardiac pat_enc_hsp_1 ON pat_enc_hsp_1.pat_enc_csn_id = procs.PAT_ENC_CSN_ID
...
USE Analytics;
:OUT \\Client\F$\clarity-cardiac\orderproc_img.rpt
SET NOCOUNT ON
SELECT PAT_ENC_HSP_1.EXTERNAL_ID CSN_ID
  ,procs.ORDER_PROC_ID
  ,procs.display_name
...
USE Analytics;
:OUT \\Client\F$\clarity-cardiac\orderproc_new.rpt
SET NOCOUNT ON
SELECT PAT_ENC_HSP_1.EXTERNAL_ID CSN_ID
  ,procs.proc_id OrderProcId
  ,procs.display_name
...
'165547', '100019', '101181', '101462', '110189', '88472', '102659', '132829', '70526', '132833', '132837', '132841', '132845', '132849', '115282', '115487', '35162', '165835', '133001', '133005', '133009', '165547', '100019', '101181', '101462', '110189', '88472', '102659', '132829', '70526', '132833', '132837', '132841', '132845', '132849', '115282', '115487', '35162', '165835', '133001', '133005', '133009' -- IABP
);
GO

:OUT \\Client\F$\clarity-cardiac\prob.rpt
SET NOCOUNT ON
...
LEFT OUTER JOIN Analytics.dbo.CCDA643_CSNLookupTable_cardiac csn ON FirstHistory.HX_PROBLEM_EPT_CSN = csn.PAT_ENC_CSN_ID
INNER JOIN Analytics.dbo.CCDA643_PatLookupTable_cardiac pat ON pat.PAT_ID = ProblemList.PAT_ID
...
USE CLARITY;
:OUT \\Client\F$\clarity-cardiac\order_narrative.rpt
SET NOCOUNT ON
select ORDER_PROC_ID, LINE, NARRATIVE, ORD_DATE_REAL, CONTACT_DATE, IS_ARCHIVED_YN
from order_narrative
where CONTACT_DATE >= '2017-04-01' and CONTACT_DATE <= '2017-05-01';
GO

USE CLARITY;
:OUT \\Client\F$\clarity-cardiac\note.rpt
SET NOCOUNT ON
SELECT DISTINCT csn.EXTERNAL_ID CSN_ID
  ,info.NOTE_ID
  ,authType.AuthorType
