IF OBJECT_ID('analytics.dbo.CCDA643_CSNLookupTable_bmc', 'U') IS NOT NULL
drop table analytics.dbo.CCDA643_CSNLookupTable_bmc;
select pat_id "PAT_ID", pat_mrn_id "PAT_MRN_ID", csn "PAT_ENC_CSN_ID", csn "EXTERNAL_ID"
INTO
analytics.dbo.CCDA643_CSNLookupTable_bmc
FROM
...
IF OBJECT_ID('analytics.dbo.CCDA643_PatLookupTable_bmc', 'U') IS NOT NULL
drop table analytics.dbo.CCDA643_PatLookupTable_bmc;
select pat_id "PAT_ID", pat_mrn_id "PAT_MRN_ID", IDENTITY_ID "EXTERNAL_ID"
INTO
analytics.dbo.CCDA643_PatLookupTable_bmc
FROM
...
INNER JOIN Analytics.dbo.CCDA643_CSNLookupTable_bmc csn ON CLARITY_ADT.PAT_ENC_CSN_ID = csn.PAT_ENC_CSN_ID
...
FROM CLARITY.dbo.PAT_ENC_HSP PAT_ENC_HSP_1
INNER JOIN CLARITY.dbo.PATIENT patient ON PAT_ENC_HSP_1.pat_id = patient.pat_id
INNER JOIN Analytics.dbo.CCDA643_CSNLookupTable_bmc csn ON PAT_ENC_HSP_1.PAT_ENC_CSN_ID = csn.PAT_ENC_CSN_ID
INNER JOIN Analytics.dbo.CCDA643_PatLookupTable_bmc pat ON pat.pat_id = pat_enc_hsp_1.pat_id
...
FROM Analytics.dbo.CCDA643_CSNLookupTable_bmc csn
...
INNER JOIN Analytics.dbo.CCDA643_CSNLookupTable_bmc csn ON pat_enc_hsp.PAT_ENC_CSN_ID = csn.PAT_ENC_CSN_ID
...
SELECT pat.EXTERNAL_ID PAT_ID
  ...
  INNER JOIN
  Analytics.dbo.CCDA643_PatLookupTable_bmc pat ON pat.PAT_ID = LDA.PAT_ID;
GO
...
inner join Analytics.dbo.CCDA643_CSNLookupTable_bmc csn ON med.PAT_ENC_CSN_ID = csn.pat_enc_csn_id
...
LEFT OUTER JOIN analytics.dbo.CCDA643_CSNLookupTable_bmc csn ON MedicalHistory.MINCSN = csn.PAT_ENC_CSN_ID
INNER JOIN analytics.dbo.CCDA643_PatLookupTable_bmc pat ON pat.PAT_ID = MedicalHistory.PAT_ID
...
INNER JOIN
Analytics.dbo.CCDA643_CSNLookupTable_bmc pat_enc_hsp_1 ON pat_enc_hsp_1.pat_enc_csn_id = med.PAT_ENC_CSN_ID
WHERE (
    med.IS_PENDING_ORD_YN = 'N'
    OR med.IS_PENDING_ORD_YN IS NULL
    )
...
INNER JOIN CCDA643_CSNLookupTable_bmc pat_enc_hsp_1 ON pat_enc_hsp_1.pat_enc_csn_id = procs.PAT_ENC_CSN_ID
...
LEFT OUTER JOIN Analytics.dbo.CCDA643_CSNLookupTable_bmc csn ON FirstHistory.HX_PROBLEM_EPT_CSN = csn.PAT_ENC_CSN_ID
INNER JOIN Analytics.dbo.CCDA643_PatLookupTable_bmc pat ON pat.PAT_ID = ProblemList.PAT_ID
