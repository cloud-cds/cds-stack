template = '''
:OUT \\\\Client\F$\clarity-all\\adt_{start_date}.rpt
SET NOCOUNT ON
...
WHERE CLARITY_ADT.EVENT_TYPE_C < 5
  ...
  AND effective_time >= '{start_date}' and effective_time < '{end_date}';
GO

:OUT \\\\Client\F$\clarity-all\demo_{start_date}.rpt
SET NOCOUNT ON
...
WHERE IDENTITY_ID.line = 1
  AND ADT_ARRIVAL_TIME >= '{start_date}' AND ADT_ARRIVAL_TIME < '{end_date}';
GO

:OUT \\\\Client\F$\clarity-all\diag_{start_date}.rpt
SET NOCOUNT ON
SELECT DISTINCT PAT_ENC_HSP.PAT_ENC_CSN_ID CSN_ID
  ,edg.DX_ID
  ,DX_ED_YN
...
WHERE ADT_ARRIVAL_TIME >= '{start_date}' AND ADT_ARRIVAL_TIME < '{end_date}';
GO

:OUT \\\\Client\F$\clarity-all\\flt_lda_{start_date}.rpt
SET NOCOUNT ON
SELECT DISTINCT pat_enc_hsp.PAT_ENC_CSN_ID CSN_ID
...
WHERE IP_FLWSHT_MEAS.RECORDED_TIME >= '{start_date}' and IP_FLWSHT_MEAS. RECORDED_TIME < '{end_date}';
GO

:OUT \\\\Client\F$\clarity-all\\flt_new_{start_date}.rpt
SET NOCOUNT ON
SELECT DISTINCT  PAT_ENC_HSP.PAT_ENC_CSN_ID CSN_ID
  ,IP_FLO_GP_DATA.FLO_MEAS_NAME
  ,IP_FLO_GP_DATA.DISP_NAME
...
) AND IP_FLWSHT_MEAS.RECORDED_TIME >= '{start_date}' and IP_FLWSHT_MEAS. RECORDED_TIME < '{end_date}';
GO

:OUT \\\\Client\F$\clarity-all\\flt_{start_date}.rpt
SET NOCOUNT ON
SELECT DISTINCT  PAT_ENC_HSP.PAT_ENC_CSN_ID CSN_ID
  ,IP_FLO_GP_DATA.FLO_MEAS_NAME
  ,IP_FLO_GP_DATA.DISP_NAME
...
:OUT \\\\Client\F$\clarity-all\labs_{start_date}.rpt
SET NOCOUNT ON
...
WHERE res.RESULT_STATUS_C IN (
  ...
  AND RES.RESULT_TIME >= '{start_date}' and RES.RESULT_TIME < '{end_date}';
GO

USE CLARITY;

:OUT \\\\Client\F$\clarity-all\labs_new_{start_date}.rpt
SET NOCOUNT ON
SELECT PAT_ENC_HSP.PAT_ENC_CSN_ID CSN_ID
  ,COMP.COMPONENT_ID
  ,COMP.BASE_NAME
...
:OUT \\\\Client\F$\clarity-all\lda_{start_date}.rpt
SET NOCOUNT ON
SELECT IDENTITY_ID PAT_ID
  ...
  WHERE IDENTITY_ID.line = 1
    AND [PLACEMENT_INSTANT] >= '{start_date}' AND [PLACEMENT_INSTANT] < '{end_date}';
GO

USE CLARITY;
:OUT \\\\Client\F$\clarity-all\mar_{start_date}.rpt
SET NOCOUNT ON
SELECT
...
AND mar.taken_time >= '{start_date}' and mar.taken_time < '{end_date}'
ORDER BY mar.TAKEN_TIME;
GO

:OUT \\\\Client\F$\clarity-all\hist_{start_date}.rpt
SET NOCOUNT ON
...
WHERE IDENTITY_ID.line = 1
  AND pat.ADT_ARRIVAL_TIME >= '{start_date}' AND pat.ADT_ARRIVAL_TIME < '{end_date}';
GO


:OUT \\\\Client\F$\clarity-all\ordermed_{start_date}.rpt
SET NOCOUNT ON
SELECT DISTINCT pat_enc_hsp.pat_enc_csn_id CSN_ID
  ,med.display_name
  ,med.ORDER_INST
...
WHERE (
  ...
  AND med.ORDER_STATUS_C NOT IN (
    ...
    ,4
    ,7
    ) AND med.ORDER_INST >= '{start_date}' AND med.ORDER_INST < '{end_date}';
GO

:OUT \\\\Client\F$\clarity-all\ordermed_home_{start_date}.rpt
SET NOCOUNT ON
SELECT DISTINCT PAT_ENC_HSP.pat_enc_csn_id CSN_ID
  ,med.display_name
  ,med.ORDER_INST
...
:OUT \\\\Client\F$\clarity-all\orderproc_{start_date}.rpt
SET NOCOUNT ON
SELECT PAT_ENC_HSP.pat_enc_csn_id CSN_ID
...
WHERE PROCS.RESULT_TIME >= '{start_date}' AND PROCS.RESULT_TIME < '{end_date}';
GO


:OUT \\\\Client\F$\clarity-all\orderproc_img_{start_date}.rpt
SET NOCOUNT ON
SELECT PAT_ENC_HSP.pat_enc_csn_id CSN_ID
  ,procs.ORDER_PROC_ID
  ,procs.display_name
...
where (img.proc_name like '%MRI%' or img.proc_name like '%CT%')
 AND PROCS.RESULT_TIME >= '{start_date}' AND PROCS.RESULT_TIME < '{end_date}';
GO

:OUT \\\\Client\F$\clarity-all\orderproc_new_{start_date}.rpt
SET NOCOUNT ON
SELECT PAT_ENC_HSP.pat_enc_csn_id CSN_ID
  ,procs.proc_id OrderProcId
  ,procs.display_name
...
:OUT \\\\Client\F$\clarity-all\prob_{start_date}.rpt
SET NOCOUNT ON
