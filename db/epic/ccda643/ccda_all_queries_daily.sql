...
WHERE IDENTITY_ID.IDENTITY_ID like 'E%'
  AND coalesce(ADT_ARRIVAL_TIME, HOSP_ADMSN_TIME) >= DATEADD(day,-7, GETDATE());
GO
...
WHERE coalesce(ADT_ARRIVAL_TIME, HOSP_ADMSN_TIME) >= DATEADD(day,-7, GETDATE());
GO
...
WHERE IDENTITY_ID.IDENTITY_ID like 'E%'
  AND coalesce(pat.ADT_ARRIVAL_TIME, pat.HOSP_ADMSN_TIME) >= DATEADD(day,-7, GETDATE());
GO
