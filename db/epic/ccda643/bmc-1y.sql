...
(SELECT DISTINCT pat_enc_hsp_1.PAT_ENC_CSN_ID, patient.PAT_ID, patient.PAT_MRN_ID
  ...
  -- ENSURE PATIENT WAS ADMITTED TO ONE OF THE HOWARD COUNTY UNITS YOU ARE STUDYING
    AND Medicalxferin.DEPARTMENT_ID like '1102%'
    ...
    AND HOSP_ADMSN_TIME >= '2016-01-01' AND HOSP_ADMSN_TIME < '2016-04-01'
...
:OUT \\Client\F$\clarity\adt.201601.rpt
SET NOCOUNT ON
...
:OUT \\Client\F$\clarity\demo.201601.rpt
SET NOCOUNT ON
...
:OUT \\Client\F$\clarity\diag.201601.rpt
SET NOCOUNT ON
SELECT DISTINCT CSN.EXTERNAL_ID CSN_ID
  ,edg.DX_ID
  ,DX_ED_YN
...
:OUT \\Client\F$\clarity\flt_lda.201601.rpt
SET NOCOUNT ON
SELECT DISTINCT csn.EXTERNAL_ID CSN_ID
...
:OUT \\Client\F$\clarity\flt_new.201601.rpt
SET NOCOUNT ON
SELECT DISTINCT  csn.EXTERNAL_ID CSN_ID
  ,IP_FLO_GP_DATA.FLO_MEAS_NAME
  ,IP_FLO_GP_DATA.DISP_NAME
...
:OUT \\Client\F$\clarity\flt.201601.rpt
SET NOCOUNT ON
SELECT DISTINCT  csn.EXTERNAL_ID CSN_ID
  ,IP_FLO_GP_DATA.FLO_MEAS_NAME
  ,IP_FLO_GP_DATA.DISP_NAME
...
:OUT \\Client\F$\clarity\labs.201601.rpt
SET NOCOUNT ON
SELECT csn.EXTERNAL_ID CSN_ID
  ,COMP.COMPONENT_ID
  ,COMP.BASE_NAME
...
:OUT \\Client\F$\clarity\labs_new.201601.rpt
SET NOCOUNT ON
SELECT csn.EXTERNAL_ID CSN_ID
  ,COMP.COMPONENT_ID
  ,COMP.BASE_NAME
...
:OUT \\Client\F$\clarity\lda.201601.rpt
SET NOCOUNT ON
...
USE CLARITY;
:OUT \\Client\F$\clarity\mar.201601.rpt
SET NOCOUNT ON
SELECT
...
:OUT \\Client\F$\clarity\hist.201601.rpt
SET NOCOUNT ON
SELECT DISTINCT csn.EXTERNAL_ID CSN_ID
...
USE Analytics;
:OUT \\Client\F$\clarity\ordermed.201601.rpt
SET NOCOUNT ON
...
USE Analytics;
:OUT \\Client\F$\clarity\ordermed_home.201601.rpt
SET NOCOUNT ON
SELECT DISTINCT PAT_ENC_HSP_1.EXTERNAL_ID CSN_ID
  ,med.display_name
  ,med.ORDER_INST
...
USE Analytics;
:OUT \\Client\F$\clarity\orderproc.201601.rpt
SET NOCOUNT ON
...
USE Analytics;
:OUT \\Client\F$\clarity\orderproc_img.201601.rpt
SET NOCOUNT ON
SELECT PAT_ENC_HSP_1.EXTERNAL_ID CSN_ID
  ,procs.ORDER_PROC_ID
  ,procs.display_name
...
USE Analytics;
:OUT \\Client\F$\clarity\orderproc_new.201601.rpt
SET NOCOUNT ON
SELECT PAT_ENC_HSP_1.EXTERNAL_ID CSN_ID
  ,procs.proc_id OrderProcId
  ,procs.display_name
...
:OUT \\Client\F$\clarity\prob.201601.rpt
SET NOCOUNT ON
...
(SELECT DISTINCT pat_enc_hsp_1.PAT_ENC_CSN_ID, patient.PAT_ID, patient.PAT_MRN_ID
  ...
  -- ENSURE PATIENT WAS ADMITTED TO ONE OF THE HOWARD COUNTY UNITS YOU ARE STUDYING
    ...
    HOSP_DISCH_TIME IS NOT NULL
    --admitted between the dates in your cohort
    AND HOSP_ADMSN_TIME >= '2016-04-01' AND HOSP_ADMSN_TIME < '2016-07-01'
) A (csn, pat_id, pat_mrn_id);
...
(SELECT DISTINCT pat_enc_hsp_1.PAT_ENC_CSN_ID, patient.PAT_ID, patient.PAT_MRN_ID
  ...
  -- ENSURE PATIENT WAS ADMITTED TO ONE OF THE HOWARD COUNTY UNITS YOU ARE STUDYING
    ...
    HOSP_DISCH_TIME IS NOT NULL
    --admitted between the dates in your cohort
    AND HOSP_ADMSN_TIME >= '2016-07-01' AND HOSP_ADMSN_TIME < '2016-10-01'
) A (csn, pat_id, pat_mrn_id);
...
(SELECT DISTINCT pat_enc_hsp_1.PAT_ENC_CSN_ID, patient.PAT_ID, patient.PAT_MRN_ID
  ...
  -- ENSURE PATIENT WAS ADMITTED TO ONE OF THE HOWARD COUNTY UNITS YOU ARE STUDYING
    ...
    HOSP_DISCH_TIME IS NOT NULL
    --admitted between the dates in your cohort
    AND HOSP_ADMSN_TIME >= '2016-10-01' AND HOSP_ADMSN_TIME < '2017-01-01'
) A (csn, pat_id, pat_mrn_id);
