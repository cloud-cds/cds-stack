:OUT \\Client\F$\clarity-all\adt_2014.rpt
SET NOCOUNT ON
...
WHERE CLARITY_ADT.EVENT_TYPE_C < 5
  ...
  AND effective_time >= '2015-01-01' and effective_time < '2015-04-01';
GO

:OUT \\Client\F$\clarity-all\demo_2014.rpt
SET NOCOUNT ON
...
WHERE IDENTITY_ID.line = 1
  AND ADT_ARRIVAL_TIME >= '2015-01-01' AND ADT_ARRIVAL_TIME < '2015-04-01';
GO

:OUT \\Client\F$\clarity-all\diag_2014.rpt
SET NOCOUNT ON
SELECT DISTINCT PAT_ENC_HSP.PAT_ENC_CSN_ID CSN_ID
  ,edg.DX_ID
  ,DX_ED_YN
...
WHERE ADT_ARRIVAL_TIME >= '2015-01-01' AND ADT_ARRIVAL_TIME < '2015-04-01';
GO

:OUT \\Client\F$\clarity-all\flt_lda_2014.rpt
SET NOCOUNT ON
SELECT DISTINCT pat_enc_hsp.PAT_ENC_CSN_ID CSN_ID
...
WHERE IP_FLWSHT_MEAS.RECORDED_TIME >= '2015-01-01' and IP_FLWSHT_MEAS. RECORDED_TIME < '2015-04-01';
GO

:OUT \\Client\F$\clarity-all\flt_new_2014.rpt
SET NOCOUNT ON
SELECT DISTINCT  PAT_ENC_HSP.PAT_ENC_CSN_ID CSN_ID
  ,IP_FLO_GP_DATA.FLO_MEAS_NAME
  ,IP_FLO_GP_DATA.DISP_NAME
...
) AND IP_FLWSHT_MEAS.RECORDED_TIME >= '2015-01-01' and IP_FLWSHT_MEAS. RECORDED_TIME < '2015-04-01';
GO

:OUT \\Client\F$\clarity-all\flt_2014.rpt
SET NOCOUNT ON
SELECT DISTINCT  PAT_ENC_HSP.PAT_ENC_CSN_ID CSN_ID
  ,IP_FLO_GP_DATA.FLO_MEAS_NAME
  ,IP_FLO_GP_DATA.DISP_NAME
...
:OUT \\Client\F$\clarity-all\labs_2014.rpt
SET NOCOUNT ON
...
WHERE res.RESULT_STATUS_C IN (
  ...
  AND RES.RESULT_TIME >= '2015-01-01' and RES.RESULT_TIME < '2015-04-01';
GO

USE CLARITY;

:OUT \\Client\F$\clarity-all\labs_new_2014.rpt
SET NOCOUNT ON
SELECT PAT_ENC_HSP.PAT_ENC_CSN_ID CSN_ID
  ,COMP.COMPONENT_ID
  ,COMP.BASE_NAME
...
:OUT \\Client\F$\clarity-all\lda_2014.rpt
SET NOCOUNT ON
SELECT IDENTITY_ID PAT_ID
  ...
  WHERE IDENTITY_ID.line = 1
    AND [PLACEMENT_INSTANT] >= '2015-01-01' AND [PLACEMENT_INSTANT] < '2015-04-01';
GO

USE CLARITY;
:OUT \\Client\F$\clarity-all\mar_2014.rpt
SET NOCOUNT ON
SELECT
...
AND mar.taken_time >= '2015-01-01' and mar.taken_time < '2015-04-01'
ORDER BY mar.TAKEN_TIME;
GO

:OUT \\Client\F$\clarity-all\hist_2014.rpt
SET NOCOUNT ON
...
WHERE IDENTITY_ID.line = 1
  AND pat.ADT_ARRIVAL_TIME >= '2015-01-01' AND pat.ADT_ARRIVAL_TIME < '2015-04-01';
GO


:OUT \\Client\F$\clarity-all\ordermed_2014.rpt
SET NOCOUNT ON
SELECT DISTINCT pat_enc_hsp.pat_enc_csn_id CSN_ID
  ,med.display_name
  ,med.ORDER_INST
...
WHERE (
  ...
  AND med.ORDER_STATUS_C NOT IN (
    ...
    ,4
    ,7
    ) AND med.ORDER_INST >= '2015-01-01' AND med.ORDER_INST < '2015-04-01';
GO

:OUT \\Client\F$\clarity-all\ordermed_home_2014.rpt
SET NOCOUNT ON
SELECT DISTINCT PAT_ENC_HSP.pat_enc_csn_id CSN_ID
  ,med.display_name
  ,med.ORDER_INST
...
:OUT \\Client\F$\clarity-all\orderproc_2014.rpt
SET NOCOUNT ON
SELECT PAT_ENC_HSP.pat_enc_csn_id CSN_ID
...
WHERE PROCS.RESULT_TIME >= '2015-01-01' AND PROCS.RESULT_TIME < '2015-04-01';
GO


:OUT \\Client\F$\clarity-all\orderproc_img_2014.rpt
SET NOCOUNT ON
SELECT PAT_ENC_HSP.pat_enc_csn_id CSN_ID
  ,procs.ORDER_PROC_ID
  ,procs.display_name
...
where (img.proc_name like '%MRI%' or img.proc_name like '%CT%')
 AND PROCS.RESULT_TIME >= '2015-01-01' AND PROCS.RESULT_TIME < '2015-04-01';
GO

:OUT \\Client\F$\clarity-all\orderproc_new_2014.rpt
SET NOCOUNT ON
SELECT PAT_ENC_HSP.pat_enc_csn_id CSN_ID
  ,procs.proc_id OrderProcId
  ,procs.display_name
...
:OUT \\Client\F$\clarity-all\prob_2014.rpt
SET NOCOUNT ON
