...
(SELECT DISTINCT pat_enc_hsp_1.PAT_ENC_CSN_ID, patient.PAT_ID, patient.PAT_MRN_ID
  ...
  -- ENSURE PATIENT WAS ADMITTED TO ONE OF THE HOWARD COUNTY UNITS YOU ARE STUDYING
    ...
    AND HOSP_ADMSN_TIME >= '2016-04-01' AND HOSP_ADMSN_TIME < '2017-05-01'
...
USE Analytics;
:OUT \\Client\F$\clarity\vent_info.201705.rpt
SET NOCOUNT ON
SELECT PAT_ENC_HSP_1.EXTERNAL_ID CSN_ID
  ,procs.proc_id OrderProcId
  ,inst.INSTNTD_ORDER_ID
...
:OUT \\Client\F$\clarity\chief_complaint.201705.rpt
SET NOCOUNT ON
SELECT DISTINCT CSN.EXTERNAL_ID CSN_ID
  ,LINE
  ,CONTACT_DATE
...
:OUT \\Client\F$\clarity\ed_events.201705.rpt
SET NOCOUNT ON
SELECT DISTINCT CSN.EXTERNAL_ID CSN_ID
  ,ein.EVENT_ID
  ,LINE
...
:OUT \\Client\F$\clarity\final_dx.201705.rpt
SET NOCOUNT ON
select
  CSN.EXTERNAL_ID CSN_ID
  ,dx.line
