...
CREATE OR REPLACE FUNCTION update_individual_metrics_for_new_discharges()
RETURNS table (
sid integer,
enc_id integer,
age real,
...
from
( select ia.enc_id, ia.sid
  from indv_alert_metrics ia
  where ia.still_in_hospital=1
) ia
join
(
  ...
  and cr.enc_id in (select ia.enc_id from indv_alert_metrics ia where ia.still_in_hospital=1)
  group by cr.enc_id
) ce
on ia.enc_id=ce.enc_id
join (select ct.enc_id, min(tsp) time_ed_admit
from cdm_t ct
...
delete from indv_alert_metrics ia
where ia.still_in_hospital=1
;
...
--- select update_individual_metrics_for_new_discharges();
