...
resource "aws_lambda_function" "dev_etl_lambda" {
    function_name    = "${var.deploy_prefix}-dev-etl-lambda"
    ...
    environment {
      variables {
        ...
        k8s_job_ETL_NAME               = "dev"
        k8s_job_ETL_SERVER             = "epic-prod"
...
resource "aws_cloudwatch_event_target" "dev_etl_schedule_target" {
    rule      = "${aws_cloudwatch_event_rule.dev_etl_schedule_rule.name}"
    target_id = "${replace(var.deploy_prefix, "-", "_")}_etl_lambda"
    arn       = "${aws_lambda_function.dev_etl_lambda.arn}"
}



resource "aws_lambda_permission" "dev_etl_cloudwatch_permissions" {
    statement_id  = "ETLPeriodicExecution"
    action        = "lambda:InvokeFunction"
    function_name = "${aws_lambda_function.dev_etl_lambda.function_name}"
