...
resource "aws_lambda_function" "dev_ops_etl_lambda_HCGH" {
    ...
    environment {
      variables {
        ...
        k8s_job_TREWS_ETL_SERVER             = "${var.DEV_OPS_ETL_SERVER}"
        k8s_job_TREWS_ETL_HOSPITAL           = "HCGH"
        k8s_job_TREWS_ETL_HOURS              = "${var.DEV_OPS_ETL_HOURS}"
        k8s_job_TREWS_ETL_ARCHIVE            = "${var.DEV_OPS_ETL_ARCHIVE}"
        k8s_job_TREWS_ETL_MODE               = "${var.DEV_OPS_ETL_MODE}"
        k8s_job_TREWS_ETL_STREAM_HOURS       = "${var.DEV_OPS_ETL_STREAM_HOURS}"
        k8s_job_TREWS_ETL_STREAM_SLICES      = "${var.DEV_OPS_ETL_STREAM_SLICES}"
        k8s_job_TREWS_ETL_STREAM_SLEEP_SECS  = "${var.DEV_OPS_ETL_STREAM_SLEEP_SECS}"
        k8s_job_TREWS_ETL_EPIC_NOTIFICATIONS = "${var.DEV_OPS_ETL_EPIC_NOTIFICATIONS}"
        k8s_job_TREWS_ETL_SUPPRESSION = "1"
      }
    }
}

resource "aws_lambda_function" "dev_ops_etl_lambda_JHH" {
...
resource "aws_lambda_function" "dev_ops_etl_lambda_BMC" {
...
resource "aws_cloudwatch_event_rule" "dev_etl_schedule_rule" {
    ...
    description = "Fires every ${var.dev_ops_etl_lambda_firing_rate_mins} minutes"
    schedule_expression = "rate(${var.dev_ops_etl_lambda_firing_rate_mins} minutes)"
}
...
resource "aws_lambda_permission" "dev_etl_cloudwatch_permissions_JHH" {
    statement_id  = "ETLPeriodicExecution"
    action        = "lambda:InvokeFunction"
    function_name = "${aws_lambda_function.dev_ops_etl_lambda_JHH.function_name}"
    principal     = "events.amazonaws.com"
    source_arn    = "${aws_cloudwatch_event_rule.dev_etl_schedule_rule.arn}"
}
...
resource "aws_lambda_permission" "etl_cloudwatch_permissions_BMC" {
    statement_id  = "ETLPeriodicExecution"
    action        = "lambda:InvokeFunction"
    function_name = "${aws_lambda_function.dev_ops_etl_lambda_BMC.function_name}"
    principal     = "events.amazonaws.com"
    source_arn    = "${aws_cloudwatch_event_rule.dev_etl_schedule_rule.arn}"
}
