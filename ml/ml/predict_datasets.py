...
def predict_test_train(inputPassedIn):
    ...
    print(time.strftime("%H:%M:%S") + " Predicting test/train for model ID {} with data ID {}".format(model_id,dataset_id))

    print(time.strftime("%H:%M:%S"))
    print(time.strftime("%H:%M:%S") + " loading dataset id:".format(dataset_id))

    factory = DataFrameFactory()
    data_train_sd = factory.load(str(model_id) + "_train_sd")
    data_test_sd = factory.load(str(model_id) + "_test_sd")

    print("loading model data id:" + str(model_id))

    mdls_f = open('%s.mdl.pkl' % model_id, 'rb')
    models = pickle.load(mdls_f)
    mdls_f.close()

    print(" Generating scores for {}: Predicting:".format(score_id))
