...
def packagePatientInfo(enc_id, adverseEventTimes, censorTimes):
    ...
    print('number of positive encounters:' + str(num_pos_patients))
    print('number of negative encounters:' + str(num_neg_patients))
    print('number of censored encounters:' + str(np.sum(idx_cens)))
    return pop_df
...
def evaluate(inputPassedIn, lambda_input='file', targetSensitivity='file', thresholds='', SubTypes='file'):
    ...
    print(time.strftime("%H:%M:%S") + " Evaluating Data ID {}".format(dataset_id))
    ...
    config = Config()
    engine = create_engine(config.get_db_conn_string_sqlalchemy())
    eventTbl = func.create_events_table(engine, dataset_id)
    engine.dispose()
    print("eventTbl:")
    print(eventTbl.head())
    ...
    for col in eventList:
        eventTbl[str(col)] = pd.to_datetime(eventTbl[str(col)], infer_datetime_format=True, errors='coerce')
    eventTbl.columns = eventList

    # eventTbl = pd.to_datetime(eventTbl, infer_datetime_format=True, errors='coerce')
    # print(eventTbl.head())
    ...
    if dataset_id in ['nomFeats9','condFeats9']:
    ...
    print("train_score.shape" + str(train_score.shape))
    print("valid_idx.shape" + str(valid_idx.shape))
    train_score_valid = train_score[valid_idx]
    print('train_score_valid.shape:' + str(train_score_valid.shape))
    ...
    for thisLambda, thisIdx in zip(lambda_clean, lambda_indexes):
        ...
        if not valid:
            print('Scores invalid for lambda ' + str(thisLambda) + ', no ROC returned')
            continue
