...
var careSummaryComponent = new function() {
  ...
  this.renderDetail = function(alert_as_cms, cms_status) {
    if ('feature_relevances' in trews.data && 'respiratory rate' in trews.data['feature_relevances']){
    ...
    var no_features_str = "";
    if ('feature_relevances' in trews.data && Object.keys(trews.data['feature_relevances']).length == 0) {
    ...
    for (var i = 0; i < phys_feats.length; i++) {
        ...
        phys_table_str += '<td>' +('feature_relevances' in trews.data && feat in trews.data['feature_relevances']? mark:"&nbsp")+'</td>';
        phys_table_str += '<td>'+(feat in displayNames ? displayNames[feat]:feat)+'</td>';

        var value = 'Not available'
        feat = feat.toLowerCase().replace(/ /g, "_");
        if ('measurements' in trews.data && feat in trews.data['measurements']) {
    ...
    for (var i = 0; i < hem_feats.length; i++) {
        ...
        hem_table_str += '<td>' +('feature_relevances' in trews.data && feat in trews.data['feature_relevances']? mark:"&nbsp")+'</td>';
    ...
    for (var i = 0; i < chem_feats.length; i++) {
        ...
        chem_table_str += '<td>' +('feature_relevances' in trews.data && feat in trews.data['feature_relevances']? mark:"&nbsp")+'</td>';
    ...
    if ( 'static_features' in trews.data ) {
