...
-- cdm stats functions --
-------------------------
create or replace function cdm_stats_fid(_dataset_id int, fid text, category text, data_type text, stats text) returns jsonb language plpgsql as $$
declare
result jsonb;
_table text;
query text;
begin
_table = 'cdm_' || lower(category);
query = 'with fval as (
  select ';
IF category = 'TWF' THEN
  query = query || fid;
elsif data_type ~* 'string' then
  query = query || 'value';
elsif data_type ~* 'json' and fid like '%_dose' then
  query = query || '(value::json->>''dose'')::real';
else
  query = query || 'value::' || data_type;
end if;
query = query || ' val from ' || _table || ' where dataset_id = ' || _dataset_id
  || ' and ';
IF category = 'TWF' THEN
  query = query || fid || '_c < 8';
else
  query = query || 'fid = ' || quote_literal(fid);
end if;
if stats = 'basic' then
  query = query || ')
  select jsonb_build_object(''count'', count(*)';
  if data_type ~* 'real|int' or (data_type ~* 'json' and fid like '%_dose') then
    query = query || '
      , ''min'', min(val),
      ''max'', max(val), ''mean'', avg(val),
      ''25%'', percentile_disc(0.25) within group (order by val),
      ''50%'', percentile_disc(0.5) within group (order by val),
      ''85%'', percentile_disc(0.85) within group (order by val)';
  end if;
  query = query ||
  '
    )
     from fval;';
elsif stats = 'histogram' then
  query = query || '),
    stats as (
    ...
    histogram as (
       select width_bucket(val, min, max, 9) as bucket,
              int4range(floor(min(val))::int, ceil(max(val))::int, ''[]'') as range,
              count(*) as freq
         from fval, stats
     group by bucket
     order by bucket
    )
    select jsonb_object_agg(''bucket'' || bucket::text,
      jsonb_build_object(''range'', range, ''freq'', freq))
       from histogram;';
end if;
...
create or replace function calculate_cdm_stats(_dataset_id int, nprocs int) RETURNS void LANGUAGE plpgsql AS $$
DECLARE
rec record;
queries text[];
query text;
BEGIN
-- cdm meta stats
for rec in select * from cdm_stats_meta where dataset_id = _dataset_id
loop
  queries = array_append(queries, 'insert into cdm_stats (dataset_id, id, id_type, stats) values
   (' || rec.dataset_id || ', ' || quote_literal(rec.id) || ', ' ||
  quote_literal(rec.id_type) || ', ' || format(rec.function, rec.dataset_id) || ') on conflict(dataset_id, id, id_type) do update set
    dataset_id = excluded.dataset_id,
    id = excluded.id,
    id_type = excluded.id_type,
    stats = cdm_stats.stats || excluded.stats');
end loop;
-- fid basic stats
for rec in select * from cdm_feature where dataset_id = _dataset_id
  and category <> 'N'
loop
 queries = array_append(queries, 'insert into cdm_stats (dataset_id, id, id_type, stats) values
   (' || rec.dataset_id || ', ' || quote_literal(rec.fid) || ', ' ||
    '''fid''' || ', ' || format('cdm_stats_fid(%s, ''%s'',''%s'', ''%s'', ''basic'')', rec.dataset_id, rec.fid, rec.category, rec.data_type) || ') on conflict(dataset_id, id, id_type) do update set
    dataset_id = excluded.dataset_id,
    id = excluded.id,
    id_type = excluded.id_type,
    stats = cdm_stats.stats || excluded.stats');
...
-- fid histogram stats
for rec in select * from cdm_feature where dataset_id = _dataset_id
  and category <> 'N' and data_type ~* 'real|int'
loop
 queries = array_append(queries, 'insert into cdm_stats (dataset_id, id, id_type, stats) values
   (' || rec.dataset_id || ', ' || quote_literal(rec.fid) || ', ' ||
    '''fid''' || ', ' || format('cdm_stats_fid(%s, ''%s'',''%s'', ''%s'', ''histogram'')', rec.dataset_id, rec.fid, rec.category, rec.data_type) || ') on conflict(dataset_id, id, id_type) do update set
    dataset_id = excluded.dataset_id,
    id = excluded.id,
    id_type = excluded.id_type,
    stats = cdm_stats.stats || excluded.stats');
...
--   raise notice 'query: %', query;
-- end loop;
perform distribute('dev_dw', queries, nprocs);
end;
$$;
